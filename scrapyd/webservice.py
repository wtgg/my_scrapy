...
from .utils import get_spider_list,get_spiders_info, JsonResource, UtilsCache, native_stringify_dict
...
class SpidersInfo(WsResource):

    def render_GET(self, txrequest):
        args = native_stringify_dict(copy(txrequest.args), keys_only=False)
        project = args['project'][0]
        version = args.get('_version', [''])[0]
        spiders_info = get_spiders_info(project, runner=self.root.runner, version=version)
        return {
            "node_name": self.root.nodename,
            "status": "ok",
            "spiders_count": len(spiders_info),
            "spiders_info": spiders_info
        }
