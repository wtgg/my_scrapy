...
def get_spiders_info(project, runner=None, pythonpath=None, version=''):
    """Return the spiders info from the given project, using the given runner"""
    if "cache" not in get_spiders_info.__dict__:
        get_spiders_info.cache = UtilsCache()
    try:
        return get_spiders_info.cache[project][version]
    except KeyError:
        pass
    if runner is None:
        runner = Config().get('runner')
    ...
    if version:
        env['SCRAPY_EGG_VERSION'] = version
    pargs = [sys.executable, '-m', runner, 'spiders_info']
    proc = Popen(pargs, stdout=PIPE, stderr=PIPE, env=env)
    out, err = proc.communicate()
    if proc.returncode:
        msg = err or out or ''
    ...
    # logger.warning(type(out_str))

    # tmp = out.decode('utf-8').splitlines();
    # logger.warning(tmp)
    # logger.warning(type(tmp))
    try:
        project_cache = get_spiders_info.cache[project]
        project_cache[version] = out_obj
    except KeyError:
        project_cache = {version: out_obj}
    get_spiders_info.cache[project] = project_cache
    return out_obj
